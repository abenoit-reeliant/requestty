...
fn prompt() -> reequestty::Result<Direction> {
    let answer = reequestty::prompt_one(
        reequestty::Question::select("direction")
...
fn exit_house() -> reequestty::Result<()> {
    loop {
        match prompt()? {
            Direction::Forward => {
...
fn encounter1() -> reequestty::Result<()> {
    loop {
        match prompt()? {
            Direction::Forward => {
...
fn encounter2a() -> reequestty::Result<()> {
    loop {
        match prompt()? {
            Direction::Forward => {
                println!(
...
fn encounter2b() -> reequestty::Result<()> {
    reequestty::prompt_one(
        reequestty::Question::select("weapon")
            .message("Pick one")
