use reequestty::{Choice, DefaultSeparator, Question, Separator};

fn main() -> reequestty::Result<()> {
    let phone_validator = regex::RegexBuilder::new(r"^([01]{1})?[-.\s]?\(?(\d{3})\)?[-.\s]?(\d{3})[-.\s]?(\d{4})\s?((?:#|ext\.?\s?|x\.?\s?){1}(?:\d+)?)?$")
        .case_insensitive(true)
        .build()
        .unwrap();

    let answers = reequestty::Answers::default();

    // the prompt module can also be created with the `reequestty::prompt_module` macro
    let mut module = reequestty::PromptModule::new(vec![
        ...
        Question::expand("toppings")
            ...
            .when(|answers: &reequestty::Answers| !answers["custom_toppings"].as_bool().unwrap())
